...
pub fn catalog() -> Result<&'static Catalog, String> {
    ...
    for entry in entries {
        match entry.kind.as_str() {
            ...
            // Thị lực giữ nguyên chuỗi trong cột `ten` và luôn gửi ID số lên HIS.
...
mod tests {
    ...
    fn visual_acuity_uses_the_exact_catalogue_name() {
        let catalog = catalog().unwrap();
        assert_eq!(visual_acuity_id(catalog, "5").unwrap(), 1952);
        assert_eq!(visual_acuity_id(catalog, "100").unwrap(), 1962);
